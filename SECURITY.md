...
EvidencePilot never intentionally logs credentials or reasoning content. `.env`, artifacts, databases, caches, and build output are ignored. URL validation blocks URL credentials, non-HTTP schemes, non-standard ports, local hostnames, private, loopback, link-local, and non-global IPs. Every redirect hop is revalidated, HTTPS downgrade redirects are rejected, environment proxies are disabled, and response media type and decoded size are bounded.
